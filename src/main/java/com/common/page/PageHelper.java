...
public class PageHelper {
	...
	private static final SensitiveWordFilter SQL_SAFE_GUARDER = new SensitiveWordFilter() {
	...
	}.addFilterWords(Arrays.asList(SQL_META));
	...
	public static String buildOrderByString(Page<?> page, String sql) {
		...
		AssertUtils.notNull(page, "page must not null");
		doSQLSafeGuard(page);
		...
		if (page.isOrderBySetted()) {
			...
			AssertUtils.isTrue(orderByArray.length == orderArray.length, "orderBy.length must equal order.length");
	...
	private static void doSQLSafeGuard(Page<?> page) {
		...
		if (page.isOrderBySetted()) {
			SQL_SAFE_GUARDER.doFilter(page.getOrderBy(), "");
		}
	}
